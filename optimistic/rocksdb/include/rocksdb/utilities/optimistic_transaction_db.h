...
// (dimitra): these are loaded for secondary cache
...
class OptimisticTransactionDB : public StackableDB {
 public:
  ...
  // (dimitra): a secondary cache that is used as locking mechanism
  std::unordered_map<std::string, int> secondaryCache; // O(1) average Insert, Deletion and Search by key
  std::mutex secondaryMutex_; // we need to atomically append the keys in the unordered_map
 protected:
