...
static uint64_t prev_recordId = 0;
static uint64_t stableWAL = 0;
static uint64_t stableTimestamp = 0;
...
namespace rocksdb {
  namespace log {
    ...
    void Reader::setInitialRecordId(uint64_t recordId) {
      prev_recordId = recordId;
      first_time = true;
    }

    void Reader::setStableWALcounter(uint64_t recordId) {
      stableWAL = recordId;
    }

    void Reader::setStableTimestamp(uint64_t recordId) {
      stableTimestamp = recordId;
    }
    ...
    bool Reader::ReadRecord(Slice* record, std::string* scratch,
      ...
      while (true) {
        ...
        size_t drop_size = 0;
        const unsigned int record_type = ReadPhysicalRecord(&fragment, &drop_size, &prev_val);
        switch (record_type) {
          case kFullType:
    ...
    bool Reader::ReadRecord(Slice* record, std::string* scratch, uint64_t* recordTimestamp, uint64_t* latestRecordId,
        WALRecoveryMode wal_recovery_mode) {
      scratch->clear();
      record->clear();
      bool in_fragmented_record = false;
      ...
      while (true) {
        uint64_t physical_record_offset = end_of_buffer_offset_ - buffer_.size();
        size_t drop_size = 0;
        const unsigned int record_type = ReadPhysicalRecord(&fragment, &drop_size, recordTimestamp, latestRecordId);
        // std::cout << *recordTimestamp << "\n";
        if (*latestRecordId <= stableWAL) {
          // continue;
        }
        else {
          std::cout <<"This record must be discarded [not stability guarantees] " << *latestRecordId << "\n";
          scratch->clear();
          record->clear();
          return false;
    ...
    unsigned int Reader::ReadPhysicalRecord(Slice* result, size_t* drop_size,
      ...
      while (true) {
        ...
        /* std::cout << "remove :" << header_size << " length: " << length << "\n"; */
        *result = Slice(header + header_size, length); // contains the record to be appended to MemTable
        if ((file_->file_name()).find("MANIFEST") == std::string::npos) {
          std::cout << file_->file_name() << "\n";
          uint64_t timestamp;
          memcpy(&timestamp, buffer_.data(), sizeof(uint64_t));
          std::cout << " Record Timestamp:  " << timestamp << "\n";
          (*previous_val)++;
          buffer_.remove_prefix(sizeof(uint64_t));
        }
        return type;
      }
    }
    ...
    unsigned int Reader::ReadPhysicalRecord(Slice* result, size_t* drop_size,
        uint64_t* previous_val, uint64_t* latestRecordId) {
      while (true) {
        // std::cout << buffer_.data() << "\n";
        // We need at least the minimum header size
        if (buffer_.size() < static_cast<size_t>(kHeaderSize)) {
        ...
        /* std::cout << "remove :" << header_size << " length: " << length << "\n"; */
        *result = Slice(header + header_size, length); // contains the record to be appended to MemTable
        if (((file_->file_name()).find("MANIFEST") == std::string::npos) && (type == kLastType || type == kFullType || type == kRecyclableLastType || type == kRecyclableFullType)) {
          // std::cout << file_->file_name() << "\n";
          uint64_t timestamp;
          memcpy(&timestamp, buffer_.data(), sizeof(uint64_t));
          // std::cout << " Record Timestamp:  " << timestamp << "\n";
          *previous_val = timestamp;
          buffer_.remove_prefix(sizeof(uint64_t));
          uint64_t recordId;
          memcpy(&recordId, buffer_.data(), sizeof(uint64_t));
          // std::cout << " Record Id:  " << recordId << "\n";
          *latestRecordId = recordId;

          // this is mocked!
          // We take the WAL's initial record Id from the Manifest
          if (first_time) {
            prev_recordId = recordId -1;
            first_time = false;
          }
          if (recordId == prev_recordId + 1) {
            prev_recordId = recordId;
          }
          else {
            std::cout << "[ERROR: Not deterministic Record Id's Increment]\n";
          }

          buffer_.remove_prefix(sizeof(uint64_t));
        }
        else if ((file_->file_name()).find("MANIFEST") == std::string::npos) {
          *latestRecordId = 0; // always be stable
          buffer_.remove_prefix(sizeof(uint64_t));
          buffer_.remove_prefix(sizeof(uint64_t));
        }
        return type;
