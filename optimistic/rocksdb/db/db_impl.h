...
//      CTSL::HashMap<std::string, int> integerMap;
      ...
      std::atomic<int> avoided_commit_waits;
    ...
    private:
      ...
      Status RecoverLogFilesWithRollbackProtection(const std::vector<uint64_t>& log_numbers,
            SequenceNumber* next_sequence, bool read_only);
      ...
      std::map<int, WriteBatch*> MergeBatchesOfSameEpoch(
            const WriteThread::WriteGroup& write_group);
      Status WriteToWALWithTimestamps(const std::map<int, WriteBatch*>* groupBatches,
