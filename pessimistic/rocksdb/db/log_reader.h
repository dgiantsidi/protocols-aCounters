  ...
  namespace log {
    ...
    class Reader {
      public:
        ...
        void setInitialRecordId(uint64_t recordId);
        void setStableWALcounter(uint64_t recordId);
        void setStableTimestamp(uint64_t recordId);
        ...
        bool ReadRecord(Slice* record, std::string* scratch, uint64_t* timestamp, uint64_t* latest_record,
            WALRecoveryMode wal_recovery_mode =
            WALRecoveryMode::kTolerateCorruptedTailRecords);
      ...
      private:
        ...
        unsigned int ReadPhysicalRecord(Slice* result, size_t* drop_size,
            uint64_t* previous_val, uint64_t* latest_record);
        // Read some more
