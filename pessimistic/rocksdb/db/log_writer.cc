  ...
  namespace log {
    ...
    Status Writer::AddRecord(const Slice& slice, const int timestamp, AsynchCounters* WALptr) {
      ...
      do {
        ...
        size_t avail = kBlockSize - block_offset_ - header_size;
        size_t fragment_length = (left + 2*sizeof(uint64_t) < avail) ? left : (avail - 2*sizeof(uint64_t));
        /* if there is not enough available space for both record data and the timestamp (that is avail >= sizeof(uint64_t) + 1) 
         * we should pad the remaining space (avail + header_size) and change block.
         */
        if (static_cast<int>(avail) <= 2*static_cast<int>(sizeof(uint64_t))) {
          s = dest_->Append(Slice("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                static_cast<size_t>(avail+header_size)));
          if (!s.ok()) {
            break;
          }
          block_offset_ = 0;
          ...
          // Invariant: we never leave < header_size bytes in a block.
          assert(static_cast<int64_t>(kBlockSize - block_offset_) >= header_size);
          avail = kBlockSize - block_offset_ - header_size;
          fragment_length = (left + 2*sizeof(uint64_t) < avail) ? left : (avail - 2*sizeof(uint64_t));
        ...
        if ((dest_->return_file_name_()).find("MANIFEST") != std::string::npos) {
          s = EmitPhysicalRecord(type, ptr, fragment_length);
          ptr += fragment_length;
          left -= fragment_length;
          begin = false;
        }
        else if ((dest_->return_file_name_()).find("MANIFEST") == std::string::npos) {
          s = EmitPhysicalRecord(type, ptr, fragment_length,
              timestamp);
    ...
    Status Writer::EmitPhysicalRecord(RecordType t, const char* ptr, size_t n,
      ...
      uint64_t timestamp = static_cast<uint64_t>(counter_id);
      char counter_ptr[sizeof(uint64_t)];
      ...
      */
      ...
      // std::cout << recordCounter.current_value() << "|\n";
      // Format the header
      buf[4] = static_cast<char>(n & 0xff);
      buf[5] = static_cast<char>(n >> 8);
      buf[6] = static_cast<char>(t);
      ...
      if (t < kRecyclableFullType) {
        // Legacy record format
        assert(block_offset_ + kHeaderSize + n + counter_size <= kBlockSize);
        header_size = kHeaderSize;
      } else {
        // Recyclable record format
        assert(block_offset_ + kRecyclableHeaderSize + n + counter_size <= kBlockSize);
        header_size = kRecyclableHeaderSize;
      ...
      if (s.ok()) {
        ...
        if (t == kRecyclableFullType || t == kRecyclableLastType || t == kFullType || t == kLastType) {
          // std::cout << "Type equals: " << t << "\n";
          s = dest_->Append(Slice(counter_ptr, counter_size));
          if (s.ok()) {
            if (!manual_flush_) {
              s = dest_->Flush();
            }
          ...
          s = dest_->Append(Slice(recordId_ptr, recordId_size));
          if (s.ok()) {
            if (!manual_flush_) {
              s = dest_->Flush();
            }
          ...
          // these are *dirty* data, when reading will be discarded
          s = dest_->Append(Slice(counter_ptr, counter_size));
          if (s.ok()) {
            if (!manual_flush_) {
              s = dest_->Flush();
      ...
      /* Increase block_offset accordingly */
      block_offset_ += header_size + n + counter_size + recordId_size;
      return s;
    }
