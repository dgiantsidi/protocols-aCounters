...
namespace rocksdb {
    ...
    Status DBImpl::WriteToWAL(const WriteBatch& merged_batch,
        ...
        *log_size = log_entry.size();
        ...
        Status status = log_writer->AddRecord(log_entry);
         /* Status status = log_writer->AddRecord(log_entry, 101); (testing purposes) */
        if (UNLIKELY(needs_locking)) {
    ...
    Status DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
        ...
        /*
         * We have a map with pointers to all groupBatches that need to be appended in the WAL.
         */
        std::map<int, class WriteBatch*> groupBatches =  MergeBatchesOfSameEpoch(write_group);
        ...
        /**
         * We set the first 8 bytes of each batch (fixed64). See write_batch.cc for 
         * batch format.
         */
        for (auto const& x : groupBatches) 
            WriteBatchInternal::SetSequence(x.second, sequence);
        
        uint64_t log_size;
        // status = WriteToWAL(*merged_batch, log_writer, log_used, &log_size);

        status = WriteToWALWithTimestamps(&groupBatches, log_writer, log_used, &log_size);
    ...
    // When two_write_queues_ is disabled, this function is called from the only
    // write thread. Otherwise this must be called holding log_write_mutex_.
    Status DBImpl::WriteToWALWithTimestamps(const std::map<int, WriteBatch*>* groupBatches,
            log::Writer* log_writer, uint64_t* log_used,
            uint64_t* log_size) {
        assert(log_size != nullptr);
        Status status;
        for (auto const& x : *groupBatches) {
            Slice log_entry = WriteBatchInternal::Contents(x.second);
    ...
    std::map<int, WriteBatch*> DBImpl::MergeBatchesOfSameEpoch(
            const WriteThread::WriteGroup& write_group) {

        std::map<int, WriteBatch*> ret;
        WriteBatch* merged_batch = nullptr;
        auto* leader = write_group.leader;
        assert(!leader->disable_wal);  // Same holds for all in the batch group
        if (write_group.size == 1 && !leader->CallbackFailed() &&
