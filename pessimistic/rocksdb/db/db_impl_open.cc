...
namespace rocksdb {
  ...
  Status DBImpl::Recover(
    ...
    if (s.ok()) {
      ...
      if (!logs.empty()) {
        ...
        s = RecoverLogFilesWithRollbackProtection(logs, &next_sequence, read_only);
        if (!s.ok()) {
  ...
  Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& log_numbers,
    ...
    for (auto log_number : log_numbers) {
      ...
      std::cout << "reading log_number:"<< log_number << "\n";
      ...
      while (!stop_replay_by_wal_filter &&
        ...
        if (!status.ok()) {
          ...
          std::cout << "[Corrupted log record found: we read valid blocks but they do not form coherent data]\n";
          reporter.Corruption(record.size(), status);
          continue;
        }
  ...
  // REQUIRES: log_numbers are sorted in ascending order
  Status DBImpl::RecoverLogFilesWithRollbackProtection(const std::vector<uint64_t>& log_numbers,
      SequenceNumber* next_sequence, bool read_only) {
    ROCKS_LOG_INFO(immutable_db_options_.info_log, "Recovering records in order timestamp");
    struct LogReporter : public log::Reader::Reporter {
      Env* env;
      Logger* info_log;
      const char* fname;
    ...
    std::map<uint64_t, std::vector<WriteBatch>> orderedBatches;
    ...
    if (prev.size() != 0)
      latest_timestamp = static_cast<uint64_t>(std::stoi(prev));
    else
      latest_timestamp = 0;
    ...
    uint64_t WALCounter = 0;
    if (prev.size() != 0)
      WALCounter = static_cast<uint64_t>(std::stoi(prev));
    ...
    for (auto log_number : log_numbers) {
      ...
      reader.setInitialRecordId(0); /* mocked for now, SPEICHER will take it from the Manifest*/
      reader.setStableWALcounter(WALCounter);
      reader.setStableTimestamp(timestamp);

      // Determine if we should tolerate incomplete records at the tail end of the
      // Read all the records and add to a memtable
      while (!stop_replay_by_wal_filter &&
          reader.ReadRecord(&record, &scratch, &timestamp, &latest_record,
            immutable_db_options_.wal_recovery_mode) &&
          status.ok()) {
        if (record.size() < WriteBatchInternal::kHeader) {
          reporter.Corruption(record.size(),
        ...
        latest_stable_record = latest_record;
        WriteBatchInternal::SetContents(&batch, record);
        SequenceNumber sequence = WriteBatchInternal::Sequence(&batch);
        std::cout << "Sequence: " << sequence << " Timestamp: " << timestamp << "\n";
        if (timestamp <= latest_timestamp)
          orderedBatches[timestamp].push_back(batch);
        if (immutable_db_options_.wal_recovery_mode ==
            WALRecoveryMode::kPointInTimeRecovery) {
          // In point-in-time recovery mode, if sequence id of log files are
          // consecutive, we continue recovery despite corruption. This could
      ...
      /*
       * We only check latest log's entry freshness. Missing/Duplicate records can be identified 
       * by the state_continuity property (previous WALid + 1 == current WALid). The initial WALid is
       * assumed to be taked by Manifest (SPEICHER offers that and also offers rollback protection for
       * MANIFEST)
       */
      ...
      for (auto i = orderedBatches.begin(); i != orderedBatches.end(); ++i) {
        std::cout << "Timestamp: " << i->first << " Size: " << i->second.size() << "\n";
        for (auto it = i->second.begin(); it != i->second.end(); ++it) {
          WriteBatch _batch = *it;
          bool has_valid_writes = false;
          status = WriteBatchInternal::InsertInto(
              &_batch, column_family_memtables_.get(), &flush_scheduler_, true,
              log_number, this, false /* concurrent_memtable_writes */,
              next_sequence, &has_valid_writes, seq_per_batch_, batch_per_txn_);
    ...
    event_logger_.Log() << "job" << job_id << "event"
      << "recovery_finished (WAL recovery in timestamp order)";
