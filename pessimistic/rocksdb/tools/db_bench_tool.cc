    ...
    public:
      ...
      void Report(int threads) {
        ...
        if (bytes_ > 0) {
          ...
          snprintf(rate, sizeof(rate), "%6.1f MB/s",
              (bytes_ / 1048576.0) / elapsed);
          rate_MB = (bytes_ / 1048576.0) / elapsed;
          extra = rate;
        }
        AppendWithSpace(&extra, message_);
        double elapsed = (finish_ - start_) * 1e-6;
        ...
        fprintf(file, "%d,%.3f,%ld,%f", threads,
            seconds_ * 1e6 / done_,
            (long)throughput, rate_MB);
        fclose(file);
      }
  };
    ...
    private:
      ...
      Stats RunBenchmark(int n, Slice name,
        ...
        for (int i = 0; i < n; i++) {
          ...
          transactions += arg[i].thread->stats.transactions_;
          aborts += arg[i].thread->stats.aborts_;
        }
        ...
        fprintf(f, "==%d,%d,", transactions, aborts);
        fclose(f);
        merge_stats.Report(n);
      ...
      void RandomTransaction(ThreadState* thread) {
        ...
        if (FLAGS_optimistic_transaction_db || FLAGS_transaction_db) {
          ...
          thread->stats.transactions_ = (int) transactions_done;
          thread->stats.aborts_ = (int) inserter.GetFailureCount();
          // fprintf(file, ",%" PRIu64 ",%" PRIu64 ",", transactions_done, inserter.GetFailureCount());
          // fclose(file);
        } else {
