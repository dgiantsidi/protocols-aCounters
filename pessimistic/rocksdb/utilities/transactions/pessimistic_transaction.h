...
class PessimisticTransaction : public TransactionBaseImpl {
 public:
  ...
  bool checkForWritesInSamePeriod();
  void putTxnKeysIntoTempCache(); 
  void write_text_to_log_file(int i, int stable, int commit);
  // the timestamp (counter's stable value)
  int counter_commit;
  ...
  void set_counter_commit(int stable_val) {
    counter_commit = stable_val;
  };
 ...
 protected:
  ...
  virtual Status  CommitWithoutPrepareInternalWithRollbackProtection(int t) = 0;
...
class WriteCommittedTxn : public PessimisticTransaction {
 ...
 private:
  ...
  Status  CommitWithoutPrepareInternalWithRollbackProtection(int t) override;
