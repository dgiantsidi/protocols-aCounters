...
class OptimisticTransaction : public TransactionBaseImpl {
 public:
  ...
  bool OptcheckForWritesInSamePeriod();
  bool AtomicOptcheckForWritesInSamePeriod();
  void OptputTxnKeysIntoTempCache();
  void Optwrite_text_to_log_file (int txnID, int stableVal);
  void lockKeys();
  bool alreadyLockedKeys();
  bool TrylockKeys();
  void unlockKeys();

  /**
   * contains the keys of a transaction in sorted order.
   * The first transaction that will append the key will block
   * any other transaction for the same key avoiding thereby fake-deadlocking
   * situations.
   */
  std::set<std::string> txnKeysSorted;
