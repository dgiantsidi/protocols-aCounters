...
namespace rocksdb {
    ...
    bool PessimisticTransaction::checkForWritesInSamePeriod() {
        const auto& tracked_keys = GetTrackedKeys();

        if (tracked_keys.size() > 0 && (tracked_keys.find(0) != tracked_keys.end())) { // if transaction is read-only we skip this
            const auto tracked_keys_cf = tracked_keys.find(0); // we consider only the default CF
            for (auto iter = tracked_keys_cf->second.begin(); iter != tracked_keys_cf->second.end(); iter++) {
                if (db_impl_->unstablePeriodCache.searchKey(iter->first)){
                    return true;
                }
            }
        }     
    ...
    void PessimisticTransaction::putTxnKeysIntoTempCache() {
        const auto& tracked_keys = GetTrackedKeys();
        if (tracked_keys.size() > 0 && (tracked_keys.find(0) != tracked_keys.end())) {
            const auto tracked_keys_cf = tracked_keys.find(0);
            for (auto iter = tracked_keys_cf->second.begin(); iter != tracked_keys_cf->second.end(); iter++){
                db_impl_->unstablePeriodCache.append(iter->first);
            }
        }  
    }

    void PessimisticTransaction::write_text_to_log_file (int txnID, int stableVal, int commitFlag) {
        ...
        log_file << commitFlag << " " << txnID << " " << stableVal << ", " << keys << "\n";
    }
    ...
    Status PessimisticTransaction::Commit() {
        ...
        if (commit_without_prepare) {
            ...
            if (WriteBatchInternal::Count(GetCommitTimeWriteBatch()) > 0) {
                ...
                int prev = db_impl_->asynch_counter->stable_value();
                // int commitWait = 0; 
                if (checkForWritesInSamePeriod()) {
                    ...
                    while (prev == db_impl_->asynch_counter->stable_value()) {;}
                }

                // these is for logging and checking the correctness of our code
                // int txnID = GetID();
                int stableVal = db_impl_->asynch_counter->stable_value();


                s = CommitWithoutPrepareInternalWithRollbackProtection(stableVal+1);

                // s = CommitWithoutPrepareInternal(); (dimitra: this was the
                // intiial case)
                if (!name_.empty()) {
                    txn_db_impl_->UnregisterTransaction(this);
                }
                ...
                /**
                 * if the transaction has been comitted successfully then we should
                 * update the index, the stats commit counter and log the txn keys into
                 * a custom log to verify our results
                 */
                if (s.ok()) {
                    std::atomic_fetch_add(&(db_impl_->num_commit), 1);
                    putTxnKeysIntoTempCache();
                    // write_text_to_log_file (txnID, stableVal+1, commitWait);
                    struct A_return_values* ret = new A_return_values;
                    // db_impl_->asynch_counter->increment(ret, stableVal+1); // we increment the asynchronous counter value
    ...
    Status WriteCommittedTxn:: CommitWithoutPrepareInternalWithRollbackProtection(int timestamp) {
        uint64_t seq_used = kMaxSequenceNumber;
        GetWriteBatch()->GetWriteBatch()->counterTimestamp = timestamp;
        //       std::cout << GetWriteBatch()->GetWriteBatch()->counterTimestamp << " EDWWW\n " << GetWriteBatch()->GetWriteBatch()->Data() << "\n";
        //
        auto s =
