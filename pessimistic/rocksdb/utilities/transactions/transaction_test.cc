...
TEST_P(MySQLStyleTransactionTest, TransactionStressTest) {
  ...
  options.write_buffer_size = 1024;
  ReOpenNoDelete();
  const size_t num_workers = 8;   // worker threads count
  const size_t num_checkers = 2;  // checker threads count
  const size_t num_transactions_per_thread = 100000;
  const uint16_t num_sets = 5;
  const size_t num_keys_per_set = 200;
